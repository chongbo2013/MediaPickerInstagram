...
public class FanProgressBar extends View {
    ...
    public FanProgressBar(Context context, AttributeSet attrs) {
        super(context, attrs);
        TypedArray typedArray = context.obtainStyledAttributes(attrs, R.styleable.JrViewFanProgressBar);
        try {
            mMaxProgress = typedArray.getFloat(R.styleable.JrViewFanProgressBar_circleMaxProgress, DEFAULT_MAX_PROGRESS);
            mProgress = typedArray.getFloat(R.styleable.JrViewFanProgressBar_circleProgress, 0);
            mInternalRadius = typedArray.getDimensionPixelSize(R.styleable.JrViewFanProgressBar_internalRadius, DEFAULT_INTERNAL_RADIUS);
            mOutRadius = typedArray.getDimensionPixelSize(R.styleable.JrViewFanProgressBar_outRadius, DEFAULT_OUT_RADIUS);
            mInternalBackgroundColor = typedArray.getColor(R.styleable.JrViewFanProgressBar_internalBackgroundColor, Color.WHITE);
            mOutBackgroundColor = typedArray.getColor(R.styleable.JrViewFanProgressBar_outBackgroundColor, Color.WHITE);
            mStartAngle = typedArray.getFloat(R.styleable.JrViewFanProgressBar_startAngle, DEFAULT_START_ANGLE);
            mInternalAlpha = (int) (255 * typedArray.getFloat(R.styleable.JrViewFanProgressBar_internalAlpha, DEFAULT_INTERNAL_ALPHA));
            mOutAlpha = (int) (255 * typedArray.getFloat(R.styleable.JrViewFanProgressBar_outAlpha, DEFAULT_OUT_ALPHA));
            mCenterX = typedArray.getFloat(R.styleable.JrViewFanProgressBar_centerX, Integer.MAX_VALUE);
            mCenterY = typedArray.getFloat(R.styleable.JrViewFanProgressBar_centerY, Integer.MAX_VALUE);
            mOutDirection = typedArray.getInt(R.styleable.JrViewFanProgressBar_outDirection, DIRECTION_CLOCKWISE);
            mInternalDirection = typedArray.getInt(R.styleable.JrViewFanProgressBar_internalDirection, DIRECTION_CLOCKWISE);
            mInitStyle = typedArray.getInt(R.styleable.JrViewFanProgressBar_initStyle, EMPTY_INIT);
        } finally {
            typedArray.recycle();
        }
        initPaint();
