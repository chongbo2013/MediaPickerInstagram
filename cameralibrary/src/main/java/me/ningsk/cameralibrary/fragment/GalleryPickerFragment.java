 ...
 * 版本：v1.0<br>
 */
public class GalleryPickerFragment extends Fragment implements MediaLoader.MediaCallBack, View.OnClickListener, FolderAdapter.OnItemClickListener, Handler.Callback,
                ...
                .subscribe(new Observer<Boolean>() {
                    ...
                    public void onNext(Boolean aBoolean) {
                        if (aBoolean) {
                            ...
                            ToastManage.s(mContext, getString(me.ningsk.mediascanlibrary.R.string.photo_jurisdiction));
                        }
                    }
    ...
    public void onClick(View v) {
        ...
        if (id == R.id.photo_left_back) {
            if (mFolderWindow.isShowing()) {
                ...
                ((CameraActivity) mContext).closeActivity();
            }
        }
        ...
        if (id == R.id.photo_right) {
            if (mMediaAdapter.getSelectedImages().size() < 1) {
                Toast.makeText(mContext, "请先选择图片或视频", Toast.LENGTH_SHORT).show();
                return;
            }
            if (isVideo) {
                mPageListener.onOpenVideoEditPage(videoInfo);
            } else {
    ...
    protected void showPleaseDialog() {
        if (!((CameraActivity) mContext).isFinishing()) {
            dismissDialog();
    ...
    public void onPictureClick(LocalMedia media, int position) {
        if (isGo) {
            if (media.getMediaMimeType() == MimeType.VIDEO) {
                ...
                isPause = false;
                resetMediaPlayer();
                Log.d("videopath", videoInfo.getPath() + "");
                try {
                    mPlayer.setDataSource(videoInfo.getPath());
